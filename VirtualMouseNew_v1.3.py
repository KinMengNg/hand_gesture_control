...
while True:
    ...
    if len(lmList) != 0:
        ...
        x1, y1 = lmList[8][1:]
        #Get corrdinate of tip of pinky
        x5, y5 = lmList[20][1:]
...
##        # print(x1, y1, x2, y2)
        ...
        if  ((left <= x1 <= right) and (top <= y1 <= bottom)) or ((left <= x5 <= right) and (top <= y5 <= bottom)):   
            ...
            # 8. Both Index and middle fingers are up ONLY: Clicking Mode
                ...
                #TO do a double click, by doing a scissors motion
                #if the length is more, and last was clicked 
                elif length >= 20 and  last_frame_clicked == True:
                    pyautogui.mouseUp()
                    last_frame_clicked = False
            ...
            #if pinky up: Scrolling
            elif fingers[1] == 0 and fingers[2] == 0 and fingers[3] == 0 and fingers[4] == 1:

                #just to indicate
                cv2.circle(img, (x5, y5), 15, (255, 0, 255), cv2.FILLED)

                #Determine  the center line of y- corrdinate in the track box
                center_y = (bottom + top)/2

                #calculate distance from center to border of the trackpad box
                #to make gradual quicker scrolling
                #the max distance from center the pinky can go
                max_dist = bottom - center_y
                dist_to_center = abs(y5 - center_y)
                max_speed = 500
                base_speed = 20
                                            #number from 0 to 1
                scroll_speed = int(base_speed + (dist_to_center/max_dist)*(max_speed))
                ngt_scroll_speed = int(-(scroll_speed))
                #if the tip of pinky above the center y, scroll up
                if y5 < center_y:
                    print('Scrolling Up..')
                    pyautogui.scroll(scroll_speed)
                    
                #if the tip of pinky below center_y, scroll down
                if y5 > center_y:
                    print('Scrolling Down..')
                    pyautogui.scroll(ngt_scroll_speed)
